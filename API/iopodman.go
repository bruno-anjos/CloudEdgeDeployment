...
type Reset_methods struct{}

func Reset() Reset_methods { return Reset_methods{} }

func (m Reset_methods) Call(c *varlink.Connection) (err_ error) {
	receive, err_ := m.Send(c, 0)
	if err_ != nil {
		return
	}
...
func (m Reset_methods) Send(c *varlink.Connection, flags uint64) (func() (uint64, error), error) {
	receive, err := c.Send("io.podman.Reset", nil, flags)
	if err != nil {
		return nil, err
	}
...
// See also [InspectImage](#InspectImage).
type ListImagesWithFilters_methods struct{}

func ListImagesWithFilters() ListImagesWithFilters_methods { return ListImagesWithFilters_methods{} }

func (m ListImagesWithFilters_methods) Call(c *varlink.Connection, filters_in_ []string) (images_out_ []Image, err_ error) {
	receive, err_ := m.Send(c, 0, filters_in_)
	if err_ != nil {
		return
	}
	images_out_, _, err_ = receive()
...
func (m ListImagesWithFilters_methods) Send(c *varlink.Connection, flags uint64, filters_in_ []string) (func() ([]Image, uint64, error), error) {
	var in struct {
		Filters []string `json:"filters"`
	}
	in.Filters = []string(filters_in_)
	receive, err := c.Send("io.podman.ListImagesWithFilters", in, flags)
	if err != nil {
		return nil, err
	}
	return func() (images_out_ []Image, flags uint64, err error) {
...
func RemoveImageWithResponse() RemoveImageWithResponse_methods {
	return RemoveImageWithResponse_methods{}
}

func (m RemoveImageWithResponse_methods) Call(c *varlink.Connection, name_in_ string, force_in_ bool) (response_out_ RemoveImageResponse, err_ error) {
	receive, err_ := m.Send(c, 0, name_in_, force_in_)
	if err_ != nil {
		return
	}
	response_out_, _, err_ = receive()
	return
}

func (m RemoveImageWithResponse_methods) Send(c *varlink.Connection, flags uint64, name_in_ string, force_in_ bool) (func() (RemoveImageResponse, uint64, error), error) {
	var in struct {
		Name  string `json:"name"`
		Force bool   `json:"force"`
	}
	in.Name = name_in_
	in.Force = force_in_
	receive, err := c.Send("io.podman.RemoveImageWithResponse", in, flags)
	if err != nil {
		return nil, err
	}
	return func() (response_out_ RemoveImageResponse, flags uint64, err error) {
		var out struct {
			Response RemoveImageResponse `json:"response"`
		}
		flags, err = receive(&out)
		if err != nil {
			err = Dispatch_Error(err)
...
func (m ImagesPrune_methods) Call(c *varlink.Connection, all_in_ bool, filter_in_ []string) (pruned_out_ []string, err_ error) {
	receive, err_ := m.Send(c, 0, all_in_, filter_in_)
	if err_ != nil {
		return
	}
...
func (m ImagesPrune_methods) Send(c *varlink.Connection, flags uint64, all_in_ bool, filter_in_ []string) (func() ([]string, uint64, error), error) {
	var in struct {
		All    bool     `json:"all"`
...
func (m InspectVolume_methods) Call(c *varlink.Connection, name_in_ string) (volume_out_ string, err_ error) {
	receive, err_ := m.Send(c, 0, name_in_)
	if err_ != nil {
		return
	}
	volume_out_, _, err_ = receive()
	return
}

func (m InspectVolume_methods) Send(c *varlink.Connection, flags uint64, name_in_ string) (func() (string, uint64, error), error) {
	var in struct {
		Name string `json:"name"`
	}
	in.Name = name_in_
	receive, err := c.Send("io.podman.InspectVolume", in, flags)
	if err != nil {
		return nil, err
	}
	return func() (volume_out_ string, flags uint64, err error) {
		var out struct {
			Volume string `json:"volume"`
		}
		flags, err = receive(&out)
...
type iopodmanInterface interface {
	...
	ListImagesWithFilters(c VarlinkCall, filters_ []string) error
	...
	ImagesPrune(c VarlinkCall, all_ bool, filter_ []string) error
	...
	InspectVolume(c VarlinkCall, name_ string) error
...
func (c *VarlinkCall) ReplyReset() error {
	return c.Reply(nil)
}
...
func (c *VarlinkCall) ReplyListImagesWithFilters(images_ []Image) error {
	var out struct {
		Images []Image `json:"images"`
	}
	out.Images = []Image(images_)
...
func (c *VarlinkCall) ReplyRemoveImageWithResponse(response_ RemoveImageResponse) error {
	var out struct {
		Response RemoveImageResponse `json:"response"`
	}
...
func (c *VarlinkCall) ReplyInspectVolume(volume_ string) error {
	var out struct {
		Volume string `json:"volume"`
	}
	out.Volume = volume_
...
// Reset resets Podman back to its initial state.
// Removes all Pods, Containers, Images and Volumes
func (s *VarlinkInterface) Reset(c VarlinkCall) error {
	return c.ReplyMethodNotImplemented("io.podman.Reset")
}
...
// after one or more filters has been applied.
// See also [InspectImage](#InspectImage).
func (s *VarlinkInterface) ListImagesWithFilters(c VarlinkCall, filters_ []string) error {
	return c.ReplyMethodNotImplemented("io.podman.ListImagesWithFilters")
}
...
// ImagesPrune removes all unused images from the local store.  Upon successful pruning,
// the IDs of the removed images are returned.
func (s *VarlinkInterface) ImagesPrune(c VarlinkCall, all_ bool, filter_ []string) error {
...
// InspectVolume inspects a single volume. Returns inspect JSON in the form of a
// string.
func (s *VarlinkInterface) InspectVolume(c VarlinkCall, name_ string) error {
	return c.ReplyMethodNotImplemented("io.podman.InspectVolume")
}
...
func (s *VarlinkInterface) VarlinkDispatch(call varlink.Call, methodname string) error {
	...
	case "Reset":
		return s.iopodmanInterface.Reset(VarlinkCall{call})
	...
	case "ListImagesWithFilters":
		...
		return s.iopodmanInterface.ListImagesWithFilters(VarlinkCall{call}, []string(in.Filters))
	...
	case "RemoveImageWithResponse":
		...
		return s.iopodmanInterface.RemoveImageWithResponse(VarlinkCall{call}, in.Name, in.Force)
	...
	case "ImagesPrune":
		...
		return s.iopodmanInterface.ImagesPrune(VarlinkCall{call}, in.All, []string(in.Filter))
	...
	case "InspectVolume":
		...
		return s.iopodmanInterface.InspectVolume(VarlinkCall{call}, in.Name)
